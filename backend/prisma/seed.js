...
async function main() {
    ...
    const quote = await prisma.quote.create({
        data: {
            number: `PRV-${new Date().getFullYear()}-0001`,
            clientId: client.id,
    ...
    if (technician) {
        const intervention = await prisma.intervention.create({
            data: {
                number: `INT-${new Date().getFullYear()}-0001`,
                clientId: client.id,
        ...
        await prisma.event.create({
            data: {
                ...
                endAt: new Date(intervention.scheduledAt.getTime() + 2 * 60 * 60 * 1000),
                clientId: client.id,
    ...
    await prisma.notification.create({
        data: {
            ...
            message: "Il sistema è stato configurato con successo. Esplora la dashboard!",
        },
    });
